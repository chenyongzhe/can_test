...
/**
 * \file kvrlib.h
 * \brief Library for interacting with remote devices. (Currently Windows only)
 * \details
 ...
 * \tableofcontents
 *
 * \section section_user_guide_kvrlib_1 Description
 * Some Kvaser devices, known as remote devices, can be connected via Ethernet
 * or WiFi, e.g. Kvaser Ethercan Light HS and Kvaser BlackBird v2, and you need
 * to configure those devices before they are connected to your computer. This
 * is where kvrlib comes into play with functions to discover and connect to a
 * Kvaser Device on the network, making it accessible for the \ref page_canlib
 * functions.
 *
 * The kvrlib has functions to configure how the remote device connects to the
 * network (e.g. dynamic/static IP). It also contains extra functions for
 * wireless setup, such as scanning and getting connection status.
 *
 * \note The kvrlib is currently only available on Windows.
 *
*/
 ...
 *
 * For an example on how to discover and connect to a remote device, see \ref example/c/kvrConnect.c
 *
 ...
 * - \ref kvrConfigClose()
 *
 * For an example on how to do local configuration, see \ref example/c/kvrConfig.c
 *
 ...
 *
 * \subpage page_user_guide_kvrlib_samples
 *
 * \section section_user_guide_kvrlib_where_to_go_from_here Where to go from here
 *
 *   For futher information check the kvrlib references in the Modules tab.
 *
 */
